...
{
	const FString DumpsDirectory = Settings->DumpsDirectory + TEXT("\\Dumps");
...
UBlueprint* FBlueprintAssetGenerator::CreateBlueprint(const FString& JsonFilePath)
{
	...
	FString ParentClassPackageName = _currentAssetInfo->GetStringField(TEXT("ParentClassPackage"));
	FString ParentClassName = _currentAssetInfo->GetStringField(TEXT("ParentClass"));
	...
	FString BlueprintPackageName = _currentAssetInfo->GetStringField(TEXT("PackageName"));
...
void FBlueprintAssetGenerator::ImportMeshesFromDirectory(const FString& PskDirectory, const FString& AssetPath)
{
	...
	for (FString& PskFilename : PskFiles)
	{
		...
		AddMaterials(
			...
			Settings->ParentForCreatedMaterials.GetLongPackageName()
		);
	}
}

UStaticMesh* FBlueprintAssetGenerator::ImportMesh(const FString& MeshName, const FString& PskFullPath, const FString& GamePath)
{
...
void FBlueprintAssetGenerator::AddMaterials(UStaticMesh* StaticMesh, const FString& MI_AssetPath, const FString& ParentMaterialPackageName)
{
	...
	const FString AssetPathToSearchForMaterialInstances = Settings->GenerateAtPath + TEXT("/Materials");

	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
	TArray<FAssetData> AssetData;
	FARFilter Filter;
	Filter.ClassNames.Add(UMaterialInstanceConstant::StaticClass()->GetFName());
	Filter.PackagePaths.Add(*AssetPathToSearchForMaterialInstances);
...
UMaterialInterface* FBlueprintAssetGenerator::CreateMaterialInstance(const FString& MaterialInstancePackageName, const FString& ParentMaterialPackageName)
{
...
{
	FString MaterialDumpsDirectory = Settings->DumpsDirectory + TEXT("\\Dumps\\Materials");
	FFileManagerGeneric FileManager;
	...
	UTexture* DiffuseTexture = FindOrImportTexture(
		TextureName,
		*DefaultTexturesPath,
		Settings->DumpsDirectory + TEXT("\\Dumps\\Textures")
	);

	return DiffuseTexture;
}

UTexture* FBlueprintAssetGenerator::FindOrImportTexture(const FString& TextureName, const FString& AssetPath, const FString& ExportedTexturesDirectory)
{
